...
## Phase 9.8: レイアウト・構造改善（完了）

- [x] **STARTエリアの左端配置**: 全体マップでSTARTバッジ付きエリアを最も左に配置。哲学（論理学・倫理学・美学）、AWS（セキュリティ・ネットワーキング）
- [x] **lessons.md統合**: ルートlessons.mdをtasks/lessons.mdに統合・削除
- [x] **コンテンツのドメイン別ディレクトリ化**: `public/content/{nodeId}/` → `public/content/{domain}/{nodeId}/`。manifest.jsonもドメイン別分割
- [x] **xx-003コンテンツ生成**: modal-logic, ecs, automata の3ノードのstandard生成完了
- [x] **AWS STARTエリア修正**: WAF/CloudFront/API Gatewayの不要なcross-area prerequisite削除 → security/networkingがSTART
- [x] **簿記レイアウト改善**: bookkeepingノードのy座標を広げてエッジの重なりを解消

## Phase 9.9: コンテンツ多言語翻訳（進行中）

- [x] **多言語翻訳機能実装**: `--locale en/zh`フラグで既存日本語コンテンツをLLMで翻訳。MDX・用語集・クイズ・SVG図解すべて対応。`content.{locale}.json`として保存
- [x] **フロントエンド対応**: `useContent`にlocale引数追加、ユーザーの言語設定に応じてlocale別ファイル取得（なければjaフォールバック）
- [x] **manifest localesフィールド追加**: レベル別利用可能言語をmanifestに記録
- [x] **エラー画面改善**: 「コンテンツが見つかりません」画面にサブマップ・全体マップの2ボタン追加
- [x] **integers クイズ生成**: 01-003のクイズ3問追加
- [x] **XX-001ノード翻訳生成**: 全6ドメインの001ノード × 2言語 = 12翻訳完了（counting, propositional-logic, ec2, binary-logic, atomic-structure, double-entry）
- [x] **XX-006ノードコンテンツ生成+翻訳**: 全6ドメインの006番ノード。日本語コンテンツ生成（sorting-searching, hydrocarbons, balance-sheet）+ 全6ノード×en/zh翻訳 = 18ファイル生成完了（set-theory, justification, vpc, sorting-searching, hydrocarbons, balance-sheet）
- [x] **`--quiz-only --locale`コンボ**: 既存翻訳ファイルにクイズ翻訳だけをマージする機能。`translateQuiz()`をexport化し、generate-content.tsに新ブランチ追加。APIコール数を9→3に削減（67%減）。中国語翻訳時のJSON内ダブルクォート未エスケープ問題も修正

## Phase 9.95: バグ修正・UX改善（完了）

- [x] **クイズ問題文の冗長表現修正**: 「コンテンツで説明されている」「本文で述べられていない」等のメタ表現を除去し、直接的な問い方に変更。対象: integers/counting/propositional-logic/double-entry の ja/en/zh。content-generator.tsのプロンプトにもメタ表現禁止ルールを追加
- [x] **MathText `<` `>` HTMLエスケープバグ修正**: `katex-utils.ts`の`renderMathInText()`で数式を先に退避→非数式部分のみHTMLエスケープ→数式をKaTeX処理して復元する方式に変更。用語一覧の `$a < 0$` が `&lt;` として表示されていた問題を修正
- [x] **コンテンツ全文検索機能**: `scripts/build-search-index.ts`でビルド時に検索インデックス生成（MDX本文+用語、クイズ除外）。`SearchBox`コンポーネントをマップヘッダーに設置。全ドメイン横断検索、ロケール対応、デバウンス付き。i18nキー追加（ja/en/zh）
- [x] **「マップに戻る」ナビゲーション修正**: エラー画面の`<Link>`を`<a href={basePath + mapUrl}>`に変更し、フルページリロードで`?area=`クエリパラメータが正しく読み取られるようにした

## Phase 9.96: マップUX改善（完了）

- [x] **AWSエリア左端配置**: STARTエリア（networking, security）をx=-200に左寄せ、非STARTエリアを右にシフト
- [x] **クイズ再受験機能**: ComprehensionCheckに「クイズに再挑戦」ボタン追加。理解済みノードでもクイズを何度でも受けられるように
- [x] **エッジ曲がり改善**: SphereEdgeのoffset計算を改善。水平/垂直配置(dy<20 or dx<20)は小offset(10)で直線的接続、斜め方向は大offset(50/80)でノード回避
- [x] **学習ページにも検索機能追加**: LearnPageClientヘッダーにSearchBoxを設置
- [x] **未履修科目の文字コントラスト改善**: locked状態のテキストをdark:text-zinc-400に、opacity-80削除
- [x] **ノードドラッグ&ドロップ**: ノードを自由にドラッグ配置可能に。位置はlocalStorageに自動保存、「レイアウトをリセット」ボタンで元に戻せる
- [x] **locked背景不透明化**: bg-zinc-800/50→bg-zinc-900に変更し、背後のエッジが透けて見える問題を解消
- [x] **全ドメインのノード間隔拡大**: 全ノード・エリアの座標を1.5倍に拡大
- [x] **左→右の難易度順レイアウト**: トポロジカル深度に基づき基礎（左）→応用（右）へ自動配置。冗長エッジ27本削除。X間隔350px、Y間隔200px
- [x] **全体マップの冗長エリア間エッジ除去**: `getAreaEdges()`に推移的到達可能性チェックを追加。A→B→Cが存在する場合の直接辺A→Cを自動除去。全ドメインに適用

## Phase 9.97: 名言（Quotes）機能追加（完了）

- [x] **Quote型定義**: `Quote { text, author, source? }` を `src/types/index.ts` に追加
- [x] **QuoteListコンポーネント**: 青左ボーダーのblockquote形式で名言を表示。名言なしの場合はnullを返却（`src/components/learn/QuoteList.tsx`）
- [x] **ContentDataにquotesフィールド追加**: `useContent`フック経由でquotes取得対応
- [x] **LearnPageClientにQuoteList統合**: 学習ページに名言セクションを表示
- [x] **生成パイプライン拡張**: `stepGenerateQuotes()`でClaude APIから実在の名言を1〜3個生成（Step 4/5と並列実行）。適切な名言がなければ空配列
- [x] **`--quotes-only`フラグ**: 既存コンテンツに名言のみ追加生成
- [x] **`--quotes-only --locale`フラグ**: 既存翻訳に名言翻訳のみマージ
- [x] **翻訳パイプライン対応**: `translateQuotes()`でquotesの多言語翻訳。`translateContent()`にも統合
- [x] **全38ノードの名言生成実行**: 22ノードに名言生成（哲学・数学で豊富、AWS・技術系は適切な名言なしで空配列）
