...
### 冗長エッジ除去はデータレベルとランタイムの2層で適用すべき
- ノードレベルの冗長エッジはビルドスクリプト（layout_nodes.py）で静的に除去
- エリアレベルの冗長エッジはランタイム（`getAreaEdges()`）で動的に除去。エリア間エッジはノードエッジから動的に導出されるため、静的除去が適用できない
- 教訓: **静的データ（JSON）と動的導出データ（関数の戻り値）では冗長エッジ除去の適用ポイントが異なる。動的導出の場合は関数内でフィルタリングする**
...
### --quiz-only --localeコンボ（翻訳の部分更新）
- クイズを`--quiz-only`で追加した後、翻訳に反映するには`--force --locale en`で全体再翻訳（4 APIコール/言語）が必要だった
- `--quiz-only --locale`コンボを追加し、既存翻訳ファイルのquizフィールドだけを差し替える機能を実装（1 APIコール/言語）
- `translateQuiz()`を個別にexportし、generate-content.tsの新ブランチから直接呼び出す
- 教訓: **翻訳パイプラインは「全体再翻訳」だけでなく「部分更新」の手段も用意すべき。コンテンツの1フィールドだけ変更した場合に全体を再実行するのはAPIコスト・時間の無駄**

### 中国語翻訳時のJSON内ダブルクォート問題
- 中国語翻訳で`"属于 $A$..."`のように引用符`"..."`がJSON文字列値内で未エスケープのまま返され、`JSON.parse`が失敗
- `parseJsonArray`の2nd attempt（文字列内文字修正）も、未エスケープの`"`で`inString`状態が壊れるため救済不可
- プロンプトに「JSON文字列内のダブルクォートは`\"`でエスケープするか、全角引用符`""`を使用」と指示して解決
- 英語では発生しにくいが中国語では「」ではなく""を使う慣習があり再現性が高い
- 教訓: **LLMにJSON出力させる際、対象言語の引用符慣習を考慮したプロンプト指示が必要。特に中国語は""（ASCII引用符）を多用するため、JSON文字列内での衝突リスクが高い**
