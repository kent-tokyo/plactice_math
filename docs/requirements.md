...
- **ノードドラッグ&ドロップ**: ノードを自由にドラッグして配置変更可能。位置はlocalStorageに自動保存（`skillmap:nodepos:{key}`）。カスタム配置がある場合は「レイアウトをリセット」ボタンが画面下部に表示。`onNodesChange` + `applyNodeChanges` で実装
- **左→右レイアウト**: トポロジカル深度に基づき左（基礎/START）→右（応用）に自動配置。冗長エッジ（推移的に到達可能な直接辺）をノードレベル（静的）・エリアレベル（`getAreaEdges()`内で動的）の両方で除去し、左→右の一貫した方向性を維持
- **エッジ描画**: `getSmoothStepPath` に動的offset。水平/垂直揃い（dy<20 or dx<20）は小offset(10)で直線的、斜め接続は大offset(50/80)でノード回避。borderRadius 20px
